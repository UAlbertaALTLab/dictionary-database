...
export default async function convertCW(inputPath, outputPath, mappingsPath) {
  ...
  if (mappingsPath) {
    ...
    for (const entry of entries) {
      ...
      const matches = mappings.get(entry.lemma.sro);

      if (!matches) continue;

      // if there are multiple matches, find the correct one and add it to the entry
      if (Array.isArray(matches)) {
        const match = matches.find(item => item.definition_MD === entry.definition);
        if (match) entry.mapping = match;
        continue;
      }

      // if there's just one match, add it to the entry
      entry.mapping = matches;
