...
class Entry {
  ...
  constructor({
    ...
    this.#extractLiteralDefinition();
    this.#extractLatinTerm();
  ...
  /**
   * Looks for Latin terms in parentheticals and saves them to the `scientificName` field on the sense.
   */
  #extractLatinTerm() {
    for (const sense of this.senses) {

      // look for a Latin definition in the parentheticals
      if (!sense.parentheticals) continue;
      const note = sense.parentheticals.find(n => n.match(Entry.#latinTermRegExp));
      if (!note) continue;

      // remove note from parentheticals list
      const i = sense.parentheticals.indexOf(note);
      sense.parentheticals.splice(i, 1);

      // set `scientificName` on sense
      sense.scientificName = note
  ...
  /**
   * Looks for any literal definitions in parentheticals and saves them to `literalMeaning` on the entry if present.
   */
  #extractLiteralDefinition() {
    for (const sense of this.senses) {

      // look for a literal definition in the parentheticals
      if (!sense.parentheticals) continue;
      const note = sense.parentheticals.find(n => n.match(Entry.#literalDefinitionRegExp));
      if (!note) continue;
      ...
      // return early if any literal definition has already been found
      return;
   ...
   * @type {RegExp}
   */
  static #literalDefinitionRegExp = /^(?:lit[:.]{1,2}|literally:?)\s+(?<literalDefinition>.+)\s*$/u;
