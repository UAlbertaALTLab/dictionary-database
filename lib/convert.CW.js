...
/**
 * Keep an in-memory Map of all the entries currently parsed.
 * (Used by the postprocessor function.)
 * @type {Map}
 */
...
function postprocessor({
  ...
  const key = createKey(standardizeSRO(sro));
  ...
  // TODO: turn this into a method on an Entry class
  ...
  const checkKey = () => {
    ...
    if (existingEntry) {
      ...
      // add homograph number 1 to it and update the map of entries.
      if (homographNum === 1) {
        entries.delete(key);
        existingEntry.key += 1;
        entries.set(existingEntry.key, existingEntry);
      }
...
export default function convertCW(inputPath, outputPath) {
  ...
  const dataStream = convert(inputPath, {
    mappings,
    ndjson:     true,
    parseError: `object`,
    postprocessor,
  ...
  dataStream.on(`finish`, () => console.log(`finish`));
